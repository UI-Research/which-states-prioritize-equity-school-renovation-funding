<script>
	...
	let yScale = scaleLinear().domain([-0.5, 1.5]).range([innerHeight, 0]);
	let yTicks = [];
	let yTicksDefault = [];
	for (let tick = 0; tick <= 2; tick += 0.1) {
		yTicksDefault.push(tick);
	}
	let pathLine;
	let data = [];
	...
	$: if ($currentState !== '') {
		...
		// make yscale symmetrical about 1, based on extent of current data
		let [yMin, yMax] = extent(data.map((d) => d.value));
		let stepOpts = [yMin, yMax].map((d) => Math.abs(1 - d));
		let biggestStep = max(stepOpts);
		let ySteps = biggestStep * 1.2;
		let domain = [1 - ySteps, 1 + ySteps];

		let yStep = max(extent(data.map((d) => d.value)).map((d) => Math.abs(1 - d)));
		yStep = yStep * 1.2;
		yScale = scaleLinear()
			.domain([1 - yStep, 1 + yStep])
			.range([innerHeight, 0]);

		yTicks = yTicksDefault.filter(
			(tick) => tick >= yScale.domain()[0] && tick <= yScale.domain()[1]
		);
